...
class VehicleUseCases {
    ...
    updateVehicle(currentCode, placa, descripcion, horometraje, kilometraje, idMarca, idModelo, callback) {
        db.query(
            "UPDATE equipo SET placa=?, descripcion=?, horometraje=?, kilometraje=?, idMarca=?, idModelo=? WHERE placa=?",
            [placa, descripcion, horometraje, kilometraje, idMarca, idModelo, currentCode],
            (err, results) => {
                if (err) {
                    return callback(err);
                }
